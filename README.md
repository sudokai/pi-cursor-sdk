...
Cloud `turn-ended`/REST usage remains display-only, but the verified Cursor SDK `Agent.getUsage()` `runs[].usage` row for the completed cloud `runId` is used for assistant token spend when available. Local rows are selected from unseen SDK usage UUIDs; cloud rows are selected by the completed `runId`. The exact billed token partition is preserved on the host-ignored `usage.cursorSdk` carrier, while overflow-visible pi fields keep uncached input within the prompt budget and keep `cacheRead`/`cacheWrite` at zero. `usage.totalTokens` is always an estimated replayable-context occupancy, floored at the latest compatible in-window assistant measurement; it is never a raw or billed aggregate total. Cursor SDK cost fields are intentionally not mapped, so pi cost remains zero/absent. The pi bridge is local-only, and pi env forwarding is not implemented yet, so `--cursor-cloud-env` forwarding-name config fails closed with Cursor-native environment setup guidance.
...
If a long-idle local session fails with “API key may be invalid or unauthorized”, a reused pooled Cursor agent can be holding an expired access token. The extension discards that agent and retries `Agent.send()` once in the same turn. If the retry also fails, the stored key was rejected — run `/login`, verify `CURSOR_API_KEY`, or pass `--api-key`. Cursor Agent CLI/Desktop login is not reused.
