...
- **Token usage separates Cursor SDK spend from occupancy.** For local turns with in-time raw `turn-ended` usage, pi records the latest per-run `inputTokens`, `outputTokens`, `cacheReadTokens`, and `cacheWriteTokens`; full-prompt `inputTokens` maps to disjoint spend components (`input = inputTokens - cacheRead - cacheWrite`, plus cache fields). Multi-invocation runs keep CSV-aligned spend fields, while `totalTokens` always uses replayable local context estimates so footer context % does not jump with billing sums. Cloud raw usage remains display-only and uses approximate pi accounting. If the SDK reports no usage in time, the extension falls back to local `input/output` activity estimates and the same occupancy path. Later usage for that live run is ignored rather than risk applying stale usage to the wrong pi turn. Cursor SDK cost is not exposed, so pi cost remains zero/absent.
