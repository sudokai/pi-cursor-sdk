...
The bridge is enabled by default when bridgeable active pi tools exist. Cursor sees bridge-owned MCP names such as `pi__sem_reindex`, while pi history and tool cards use the real pi tool name such as `sem_reindex`. The bridge hides overlapping built-in pi tools by default because Cursor already has native equivalents; extension/custom tools and non-overlapping active tools present in pi's active tool registry normally remain exposed. pi-cursor-sdk registers `cursor_ask_question` for Cursor models only when `PI_CURSOR_ASK_QUESTION=1` (with the bridge on and a Cursor model); it is off and unregistered by default. When enabled, Cursor sees it as `pi__cursor_ask_question`, so Cursor can ask the user to choose instead of silently defaulting when the pi UI is available. When pi has Agent Skills loaded, pi-cursor-sdk registers `cursor_activate_skill`, exposed as `pi__cursor_activate_skill`, so Cursor can load the full pi `SKILL.md` for a catalog skill or a user-invoked `/skill:name` (including `disable-model-invocation` skills omitted from the catalog). The bridge does not call pi tool `execute()` handlers directly; it queues the request, emits a real pi `toolCall`, waits for the matching pi `toolResult`, and resolves the Cursor MCP call back into the same live Cursor SDK run without creating a new `Agent`, unless the run was disposed, aborted, or cancelled.
...
Bridged pi tool calls follow the same visible pi `toolUse` turn shape, but they are real pi tool executions rather than replayed Cursor results. Usage accounting separates spend from occupancy: structurally valid local raw `turn-ended` usage maps full-prompt input into uncached pi `input` (`input = inputTokens - cacheReadTokens - cacheWriteTokens`, capped at the prompt budget) and zeroes the overflow-visible `cacheRead`/`cacheWrite` pi fields, carrying the real SDK billing (including multi-invocation sums) on the host-ignored `usage.cursorSdk` carrier so pi-ai's `input + cacheRead` silent-overflow check never fires on healthy turns. Verified `Agent.getUsage()` billed rows use that same safe visible projection and take precedence when present: local rows are selected by unseen usage UUIDs and cloud rows by completed `runId`, while the exact billed partition remains on `usage.cursorSdk`. `usage.totalTokens` always uses replayable-context occupancy estimation, floored at the latest compatible in-window assistant measurement; it never uses raw or billed SDK aggregates. Cloud raw turn-ended/REST usage remains display-only when no billed SDK row is available and uses approximate pi message accounting. Cursor SDK cost fields are intentionally not mapped to pi cost. If the SDK reports no usage in time, the provider falls back to local `input/output` activity estimates with the same occupancy path, then ignores later usage for that live run rather than risk applying stale usage to the wrong pi turn. Per-turn usage is recorded as the latest event, never summed.
