...
Bridged pi tool calls follow the same visible pi `toolUse` turn shape, but they are real pi tool executions rather than replayed Cursor results. Usage accounting splits spend from occupancy: structurally valid SDK `turn-ended` usage maps the full prompt into disjoint pi spend components (`input = inputTokens - cacheReadTokens - cacheWriteTokens`, plus `cacheRead` / `cacheWrite` / `output`), including multi-invocation billing sums; `usage.totalTokens` is always a local replayable-context estimate (never SDK billing). If the SDK reports no usage in time, the provider falls back to local `input/output` activity estimates with zero cache fields and the same occupancy estimate, then ignores later usage for that live run rather than risk applying stale usage to the wrong pi turn. Per-turn usage is recorded as the latest event (never summed). The SDK emits one `turn-ended` per run at completion; the final turn briefly reconciles for it before the run is marked finished (`PI_CURSOR_TURN_ENDED_WAIT_MS`, default 500 ms) so the per-run spend applies to the stop turn instead of racing to approximate.
