...
- Cursor SDK usage events are used when the SDK reports them before the corresponding pi turn is emitted and the reported counts fit the selected pi model window. For each safe SDK-attributed assistant turn, `usage.input`, `usage.output`, `usage.cacheRead`, and `usage.cacheWrite` come from the latest per-turn SDK usage; the SDK reports `inputTokens` as the full prompt (it includes the `cacheRead` and `cacheWrite` shares), so pi stores the disjoint components as `input = inputTokens - cacheReadTokens - cacheWriteTokens`, `cacheRead`, and `cacheWrite`. The Cursor usage-events CSV `Total Tokens` column matches that billing decomposition (`inputTokens + outputTokens`); do not sum all four SDK fields — that double-counts the cache share. **Occupancy** (`usage.totalTokens` for footer/compaction) uses that billing total only for single-invocation runs (`onStep` `assistantMessage` count ≤ 1). When a run makes multiple model invocations, the one end-of-run `turn-ended` usage is a billing sum across invocations (same totals as the CSV) and must not be treated as window fill — occupancy then takes the max of the replayable context estimate, the last accepted assistant occupancy, and the per-invocation mean of the billing total. Approximate fallback never reports less occupancy than the last accepted assistant measurement in context. Cumulative `RunResult.usage` is never used for per-message occupancy. If the SDK reports no usage in time, or reports full-agent-context-sized usage outside the selected model window, the provider falls back to local `input/output` activity estimates while setting `usage.totalTokens` to the current replayable context estimate so footer/compaction context does not collapse after split tool turns. Per-turn SDK usage is recorded as the latest event (never summed across turns); after a split live-run turn times out waiting for SDK usage, later usage is ignored rather than risk applying stale usage to the wrong pi turn. The SDK emits exactly one `turn-ended` per run at completion (~10–150 ms after `step-completed`); because `run.wait()` resolves at the same instant, the final turn briefly reconciles for that event before the run is marked finished (override via `PI_CURSOR_TURN_ENDED_WAIT_MS`, default 500 ms) so the one real per-run usage is applied to the stop turn instead of racing to approximate. Intermediate split turns have no SDK usage of their own and fall back to the local estimate. Cursor SDK cost is unavailable, so cost remains absent/zero. `src/cursor-usage-accounting.ts` owns the safety/approximate/multi-invocation occupancy policy; `src/cursor-live-run-coordinator.ts` owns the turn-ended reconcile and `assistantMessage` step counting. See `docs/investigations/cursor-live-run-turn-ended-usage-2026-07-27.md` for the measurement basis.
