...
- Cursor SDK usage splits **spend** from **occupancy**. Local raw `turn-ended` usage is used when structurally valid and arrives before the corresponding pi turn is emitted: full-prompt `inputTokens` maps to uncached `input` (`input = inputTokens - cacheReadTokens - cacheWriteTokens`, capped at the prompt budget). SDK `cacheReadTokens`/`cacheWriteTokens` are a billing sum across invocations (verified against the Cursor usage-events CSV) — valid spend, never context occupancy. pi-ai's silent-overflow check reads `usage.input + usage.cacheRead` as prompt size, so pi-cursor-sdk zeroes the overflow-visible `cacheRead`/`cacheWrite` pi fields and carries the real SDK billing on the host-ignored `usage.cursorSdk` carrier (`{ inputTokens, outputTokens, cacheReadTokens, cacheWriteTokens }`). **Occupancy** (`usage.totalTokens` for compaction) is always a local replayable-context estimate, floored at the last compatible same-model in-window assistant occupancy — never SDK billing. Cloud raw usage remains display-only. Approximate fallback uses local `input/output` estimates and the same occupancy path. Cumulative `RunResult.usage` is never used for per-message spend or occupancy. Per-turn SDK usage is recorded as the latest event (never summed across turns); after a split live-run turn times out waiting for SDK usage, later usage is ignored rather than risk applying stale usage to the wrong pi turn. Cursor SDK cost is unavailable, so cost remains absent/zero. `src/cursor-usage-accounting.ts` owns the spend/occupancy split; `src/cursor-live-run-coordinator.ts` owns turn-ended reconciliation.
