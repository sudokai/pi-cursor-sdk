...
- Cursor SDK usage splits **spend** from **occupancy**. Spend (`usage.input` / `output` / `cacheRead` / `cacheWrite`) comes from the latest structurally valid per-run SDK `turn-ended` usage when it arrives before the corresponding pi turn is emitted: the SDK reports `inputTokens` as the full prompt (it includes the `cacheRead` and `cacheWrite` shares), so pi stores the disjoint components as `input = inputTokens - cacheReadTokens - cacheWriteTokens`, `cacheRead`, and `cacheWrite`. The Cursor usage-events CSV `Total Tokens` column matches that billing decomposition (`inputTokens + outputTokens`); do not sum all four SDK fields — that double-counts the cache share. Multi-invocation runs emit one end-of-run billing sum; that sum is still valid spend (footer ↑/↓/R/CH). **Occupancy** (`usage.totalTokens` for compaction) is always a local replayable-context estimate, floored at the last accepted assistant occupancy — never SDK billing. Approximate fallback (no/`invalid` `turn-ended`) uses local `input/output` estimates with `cacheRead = cacheWrite = 0` and the same occupancy estimate path. Cumulative `RunResult.usage` is never used for per-message spend or occupancy. Per-turn SDK usage is recorded as the latest event (never summed across turns); after a split live-run turn times out waiting for SDK usage, later usage is ignored rather than risk applying stale usage to the wrong pi turn. The SDK emits exactly one `turn-ended` per run at completion (~10–150 ms after `step-completed`); because `run.wait()` resolves at the same instant, the final turn briefly reconciles for that event before the run is marked finished (override via `PI_CURSOR_TURN_ENDED_WAIT_MS`, default 500 ms) so the one real per-run usage is applied to the stop turn instead of racing to approximate. Intermediate split turns have no SDK usage of their own and fall back to the local estimate. Cursor SDK cost is unavailable, so cost remains absent/zero. `src/cursor-usage-accounting.ts` owns the spend/occupancy split; `src/cursor-live-run-coordinator.ts` owns the turn-ended reconcile. See `docs/investigations/cursor-live-run-turn-ended-usage-2026-07-27.md` for the measurement basis of the reconcile race.
