...
- After each finished SDK run, the provider calls the verified `agent.getUsage()` contract. Local billed rows are selected from unseen SDK usage UUIDs; cloud billed rows are selected by the completed cloud `runId`. A valid billed row takes precedence for token spend, but its exact `{ inputTokens, outputTokens, cacheReadTokens, cacheWriteTokens }` partition stays on host-ignored `usage.cursorSdk`; visible pi `input` is the uncached component capped at the prompt budget and visible cache fields stay zero so installed pi-ai `isContextOverflow()` cannot treat cumulative billing as prompt size. `usage.totalTokens` always uses estimated replayable-context occupancy, floored at the latest compatible in-window assistant measurement; raw and billed aggregate totals never become occupancy. Cloud raw turn-ended/REST usage remains display-only when no billed row is available, and Cursor SDK cost fields are intentionally not mapped to pi cost (zero/absent).
...
- The provider registers `cursor_ask_question` for Cursor models only when `PI_CURSOR_ASK_QUESTION=1` (it is off and unregistered by default); Cursor then sees it as `pi__cursor_ask_question` (also requires the bridge enabled and a Cursor model). `PI_CURSOR_ASK_QUESTION=0` (or any disabling value) keeps it unregistered while preserving the rest of the bridge. While the tool awaits pi UI input it emits package event `pi-cursor-sdk:ask-question:blocked` with `{ active: true }` and clears `{ active: false }` in `finally`; the tool runs with `executionMode: "sequential"` so parallel sibling calls cannot overlap dialogs. When active, pi executes it through the normal tool path so interactive users can choose options from pi UI. In non-UI modes it reports that UI is unavailable so Cursor can state a default assumption instead. When pi has Agent Skills loaded, the provider rewrites the skill catalog for Cursor (omitting `disable-model-invocation` skills) and registers `cursor_activate_skill` as `pi__cursor_activate_skill`; pi executes it through the normal tool path so Cursor can load the full `SKILL.md` and skill resource list for a catalog skill or a user-invoked `/skill:name`. `PI_CURSOR_PI_TOOL_BRIDGE=0` disables the local bridge, including question and skill activation bridging. Cloud Cursor agents remain out of scope for the bridge.
...
- Cursor SDK usage splits **spend** from **occupancy**. Structurally valid local raw `turn-ended` usage remains the latest per-run event: full-prompt `inputTokens` maps to uncached `input` (`input = inputTokens - cacheReadTokens - cacheWriteTokens`, capped at the prompt budget), while the SDK cache fields remain on host-ignored `usage.cursorSdk` because they are billing sums across invocations (verified against the Cursor usage-events CSV), never context occupancy. Verified `Agent.getUsage()` billed rows use the same safe pi-visible projection and take precedence when present; local rows are selected by unseen usage UUIDs and cloud rows by the completed `runId`. The exact billed partition remains on `usage.cursorSdk`, and pi-ai's `isContextOverflow()` sees zero visible cache fields. **Occupancy** (`usage.totalTokens` for compaction) always uses a replayable-context estimate, floored at the last compatible same-model in-window assistant occupancy. Raw and billed SDK aggregates never become occupancy. Cloud raw turn-ended/REST usage remains display-only when no billed SDK row is available; approximate fallback uses local `input/output` estimates and the same occupancy path. Cumulative `RunResult.usage` is never used for per-message spend or occupancy. Per-turn SDK usage is recorded as the latest event (never summed across turns); after a split live-run turn times out waiting for SDK usage, later usage is ignored rather than risk applying stale usage to the wrong pi turn. Cursor SDK cost fields are intentionally unmapped, so cost remains absent/zero. `src/cursor-usage-accounting.ts` owns the spend/occupancy split; `src/cursor-live-run-coordinator.ts` owns turn-ended reconciliation.
